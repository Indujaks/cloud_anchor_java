...
Log.e(TAG,"indu oncreate");
        ...
        if (anchor == null
          ...
          for (HitResult hit : frame.hitTest(queuedSingleTap)) {
            if (shouldCreateAnchorWithHit(hit)) {
              ...
              Log.e(TAG,"indu hostcloudanchor");
  ...
  private static boolean shouldCreateAnchorWithHit(HitResult hit) {
    ...
    Log.e(TAG,"indu should create anchor with hit");
  ...
  public void onSurfaceCreated(GL10 gl, EGLConfig config) {
    ...
    Log.e(TAG,"indu onsurfacecreate");
  ...
  public void onSurfaceChanged(GL10 gl, int width, int height) {
    Log.e(TAG,"indu onsurface changed");
    ...
    try {
     ...
     /* if (cameraTrackingState == TrackingState.PAUSED) {
      ...
      for (ColoredAnchor coloredAnchor : anchors) {
        ...
        //Log.e(TAG,"indu: "+coloredAnchor.anchor.toString());
        // Update and draw the model and its shadow.
        virtualObject.updateModelMatrix(anchorMatrix, scaleFactor);
        virtualObjectShadow.updateModelMatrix(anchorMatrix, scaleFactor);
        //Log.e(TAG,"indu: view matrix"+ viewMatrix +"projection matrix "+projectionMatrix);
        virtualObject.draw(viewMatrix, projectionMatrix, colorCorrectionRgba, coloredAnchor.color);
        virtualObjectShadow.draw(viewMatrix, projectionMatrix, colorCorrectionRgba, coloredAnchor.color);
      }
...
Log.e(TAG,"indu shouldDrawAnchor value"+shouldDrawAnchor);
      // Visualize anchor.
      if (shouldDrawAnchor) {
        float[] colorCorrectionRgba = new float[4];
        frame.getLightEstimate().getColorCorrection(colorCorrectionRgba, 0);
  ...
  private void setNewAnchor(Anchor newAnchor) {
    Log.e(TAG,"indu setnewanchor");
    float[] objColor=new float[] {66.0f, 133.0f, 244.0f, 255.0f};
    anchors.add(new ColoredAnchor(newAnchor,objColor));
    /*synchronized (anchorLock) {
      if (anchor != null) {
        //anchor.detach();
      }
  ...
  private void onHostButtonPress() {
    ...
    Log.e(TAG,"indu onhostbuttonpress");
  ...
  private void onResolveButtonPress() {
    Log.e(TAG,"indu onresolvebuttonpress");
  ...
  private void onRoomCodeEntered(Long roomCode) {
    Log.e(TAG,"indu onroomcodeentered");
    currentMode = HostResolveMode.RESOLVING;
  ...
  private final class RoomCodeAndCloudAnchorIdListener
    ...
    public void onCloudTaskComplete(Anchor anchor) {
      Log.e(TAG,"indu oncloudtaskcomplete");
      CloudAnchorState cloudState = anchor.getCloudAnchorState();
      if (cloudState.isError()) {
